...
def generate() -> str:
    ...
    wizard_count = sum(
        1 for s in SOURCE_REGISTRY.values() if s.get("wizard_enabled")
    )
    coming_soon = [
        k for k, v in SOURCE_REGISTRY.items()
        if not v.get("wizard_enabled") and k not in ("csv", "filesystem")
    ]
    hidden = [k for k in SOURCE_REGISTRY if k in ("csv", "filesystem")]
    ...
    for cat, source_order in CATEGORY_SOURCE_ORDER.items():
        ...
        cat_wizard = sum(
            ...
            if SOURCE_REGISTRY[s].get("wizard_enabled")
        )
        cat_coming = sum(
